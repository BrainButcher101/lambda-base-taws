...
locals {
  ...
  function_name = "${local.env}-${var.function.name}"
...
resource "aws_lambda_function" "lambda" {
  ...
  filename         = var.l_function.zip
  source_code_hash = local.function.hash
  layers           = local.layer == null ? [] : [aws_lambda_layer_version.layer[0].arn]
  handler          = var.l_function.handler
  role             = try(var.l_function.role.arn, aws_iam_role.lambda.arn)
  memory_size      = local.function.memsize
  runtime          = var.l_function.runtime
